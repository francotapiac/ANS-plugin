...
"""
def get_name_signal( type_signal):
    if(type_signal == 1):
        return "ECG"
    else:
        return "PPG"
...
"""
def get_rpeaks(points_signal, type_signal, sampling):
    if(type_signal== 1):
...
"""
def create_windows_signal( signal, t, window_size, shif):
    signal_segment = []
    t_segment = []
    init_pos = 0
...
def freqTimeToDataframe(time_ecg,freq_ecg):
...
@return:        modelo
"""
def read_model_prediction(path):
    model = load(path)
    return model
...
"""
def process_signal(signal_segment, t_segment, type_signal, sampling):
    time_line = []
    start_time = 0
    for segment, time in zip(signal_segment,t_segment):
        ...
        t,filtered_signal,rr_peaks = get_rpeaks(segment, type_signal, sampling)
        nni = get_nni(rr_peaks, t)

        # Obteniendo atributos del dominio de la frecuencia y tiempo de la señal
        time_params = get_time_params(rr_peaks,t)
        freq_params = get_freq_params(rr_peaks,t)
        time_df, freq_df = freqTimeToDataframe(time_params,freq_params)
        # Calculando la coherencia cardíaca
        ratio_coherence = get_cardiac_coherence(nni)
        #Obteniendo emoción
        ...
        model = read_model_prediction(path_model)
